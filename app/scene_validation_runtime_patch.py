"""Pre-commit scene validation and automatic rewrite gate for Akira 1206 v3."""
...
from app import production_runtime_patch as production
from app import v3_apply_turn_result_runtime_patch as writer
...
production.RUNTIME_VERSION = VERSION
writer.RUNTIME_VERSION = VERSION
if sys.modules.get("app.context_request_runtime_patch"):
    sys.modules["app.context_request_runtime_patch"].RUNTIME_VERSION = VERSION
app.version = VERSION

KNOWN_IDS = set(writer.ID_ALIASES.values())
NAMES = {
...
REQUIRED_SAFETY = {
    ...
    "clock_changed_only_through_elapsed_minutes", "npc_routes_respect_eta",
...
def _snapshot_ids(snapshot: dict[str, Any]) -> set[str]:
    keys = {
        "character_ids", "active_character_ids", "scene_character_ids",
        "present_character_ids", "memory_character_ids", "addressed_character_ids",
        "speaking_character_ids", "remote_contact_character_ids", "contacted_character_ids",
    }
    return {writer._cid(item) for value in _collect(snapshot, keys) for item in _list(value) if writer._cid(item)}


def _pov(snapshot: dict[str, Any], pending: dict[str, Any]) -> str:
    for value in _collect(snapshot, {"pov_character_id"}):
        if writer._cid(value):
            return writer._cid(value)
    patch = pending.get("current_state_patch") if isinstance(pending.get("current_state_patch"), dict) else {}
    return writer._cid(patch.get("pov_character_id") or "akira") or "akira"
...
def _speakers(text: str, validation: dict[str, Any]) -> set[str]:
    ...
    return {writer._cid(label.strip().strip(":：")) for label in labels if writer._cid(label.strip().strip(":：")) in KNOWN_IDS}


def _last_scene(sid: str) -> str:
    history = base.read_session_json(writer.SCENE_HISTORY_FILE, sid, default=[])
...
def validate_scene(sid: str, body: dict[str, Any], payload: dict[str, Any], turn_id: str, text: str) -> dict[str, Any]:
    pending = base.get_pending_turn(sid) or {}
    snapshot = base.read_session_json(base.CONTEXT_SNAPSHOT_FILE, sid, default={})
    ...
    if loaded:
        for cid in sorted(speaker_ids - loaded):
            errors.append(_error("unloaded_character_spoke", f"Character '{cid}' spoke outside the frozen snapshot.", character_id=cid, loaded_character_ids=sorted(loaded)))
    ...
    if isinstance(addressed, dict):
        for raw, responded in addressed.items():
            if responded is not True:
                errors.append(_error("addressed_character_left_without_response", "Directly addressed character has no visible response/refusal/reaction.", character_id=writer._cid(raw) or str(raw)))
...
def _attempt(body: dict[str, Any], payload: dict[str, Any]) -> int:
...
def _rewrite(sid: str, turn_id: str, result: dict[str, Any], attempt: int) -> dict[str, Any]:
...
_original_apply = writer.apply_turn_result_v3
writer._remove_route(writer.APPLY_PATH, "POST")
writer._remove_route("/health", "GET")


@app.get("/health", operation_id="health")
def health_with_scene_gate() -> dict[str, Any]:
    return {**production.health(), "version": VERSION, "scene_validation_protocol": PROTOCOL, "automatic_scene_rewrite_attempts": MAX_REWRITES}


@app.post(writer.APPLY_PATH, operation_id="applyTurnResult")
def apply_with_scene_gate(session_id: str, body: dict[str, Any] | None = Body(default=None)) -> dict[str, Any]:
    sid = writer._safe_session_id(session_id); base.ensure_session(sid)
    body = body if isinstance(body, dict) else {}
    payload = writer._payload(body)
    turn_id = writer._turn_id(body, payload)
    text = writer._scene_text(body, payload)
    if not turn_id or not text:
        return _original_apply(session_id, body)

    runtime = base.read_turn_runtime(sid)
    last = runtime.get("last_applied_turn")
    if isinstance(last, dict) and last.get("turn_id") == turn_id:
        replay = _original_apply(session_id, body)
        if isinstance(replay, dict):
            replay = dict(replay); replay.setdefault("scene_validation", {"passed": True, "protocol": PROTOCOL, "idempotent_replay": True})
        return replay
    pending = runtime.get("pending_turn")
    snapshot = base.read_session_json(base.CONTEXT_SNAPSHOT_FILE, sid, default={})
    if not isinstance(pending, dict) or pending.get("turn_id") != turn_id or not isinstance(snapshot, dict) or not snapshot.get("all_required_chunks_served"):
        return _original_apply(session_id, body)

    validation = validate_scene(sid, body, payload, turn_id, text)
    if not validation["passed"]:
        return _rewrite(sid, turn_id, validation, _attempt(body, payload))
    result = _original_apply(session_id, body)
    if isinstance(result, dict):
        result = dict(result)
        result["scene_validation"] = {"passed": True, "protocol": PROTOCOL, "warnings": validation["warnings"], "checks": validation["checks"]}
        if result.get("status") == "applied":
            result["display_instruction"] = "State and scene validation passed. Show visible_scene_text only; hide validation and internal JSON."
    return result


_original_openapi = production.openapi_actions


def _openapi() -> dict[str, Any]:
    schema = json.loads(json.dumps(_original_openapi(), ensure_ascii=False))
    schema["info"]["version"] = VERSION
    schema["info"]["description"] += " applyTurnResult validates the draft before commit and returns a same-turn rewrite packet on failure."
    operation = schema["paths"][writer.APPLY_PATH]["post"]
    operation["summary"] = "Validate, request automatic rewrite when needed, then atomically apply the pending turn."
    properties = operation["requestBody"]["content"]["application/json"]["schema"].setdefault("properties", {})
    properties["safety_checks"] = {"type": "object", "properties": {key: {"type": "boolean"} for key in sorted(REQUIRED_SAFETY)}, "additionalProperties": True}
    properties["continuity_checks"] = {"type": "object", "additionalProperties": {"type": "boolean"}}
    properties["scene_validation"] = {
        "type": "object", "additionalProperties": True,
        "properties": {
            "repair_attempt": {"type": "integer", "minimum": 0},
            "speaker_character_ids": {"type": "array", "items": {"type": "string"}},
            "addressed_character_responses": {"type": "object", "additionalProperties": {"type": "boolean"}},
        },
    }
    return schema


production.openapi_actions = _openapi
