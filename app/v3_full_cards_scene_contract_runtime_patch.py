...
It never uses short character summary files as a behavior source and never falls back to Akira just because a scene has no selected ids. Hidden past is included only when a scene trigger is present.
"""
...
def build_v3_scene_contract_response(session_id: str, *, max_total_chars: int = 18000, include_debug: bool = False) -> dict[str, Any]:
    ...
    contract: dict[str, Any] = {
        ...
        "forbidden_context": [
            "No short character summary files as behavior source.",
            "Dynamic character memory source is state/character_memory/<id>.json only.",
...
def get_context_audit_v3_full_cards(
    ...
    return {
        ...
        "instructions": [
            ...
            "Characters must come from full v3 cards, not short summaries.",
